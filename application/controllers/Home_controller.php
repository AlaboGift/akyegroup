...
class Home_controller extends Home_Core_Controller
{
    ...
    public function index()
    {
        ...
        $data["products"] = $this->product_model->get_products_limited($this->general_settings->index_latest_products_count);
        $data["promoted_products"] = $this->product_model->get_promoted_products_limited(6);
        ...
        $data['mobile_cat']=$this->category_model->get_mobile_category();
        $data['country_id']=$this->get_country_id();
        //reset sort value
        if (!empty($_SESSION["modesy_sort_products"])) {
            $_SESSION["modesy_sort_products"] = null;
        }
    ...
    public function product($slug)
    {
        ...
        if (empty($data['product'])) {
            ...
            if ($data['product']->status == 0 || $data['product']->visibility == 0) {
                if (!auth_check() || (auth_check() && $data['product']->user_id != user()->id)) {
                    redirect($this->agent->referrer());
                }
            }
            ...
            $data['og_url'] = base_url() . $data['product']->slug;
            $data['og_image'] = get_product_main_image($data['product']->id);
            $data['og_width'] = "750";
            $data['og_height'] = "500";
            if (!empty($data['user'])) {
    ...
    public function load_more_promoted_products()
    {
        ...
        $data["promoted_products"] = $this->product_model->load_more_promoted_products($this->promoted_products_limit, $limit);
    ...
    */
    ...
    public function blog()
    {
        ...
        //get paginated posts
        $pagination = $this->paginate(base_url() . 'blog', $this->blog_model->get_posts_count(), $this->blog_paginate_per_page);
    ...
    public function category($slug)
    {
        ...
        //get paginated posts
        $pagination = $this->paginate(base_url() . 'blog/' . $data["category"]->slug, count($this->blog_model->get_posts_by_category($data["category"]->id)), $this->blog_paginate_per_page);
    ...
    public function tag($slug)
    {
        ...
        //get paginated posts
        $pagination = $this->paginate(base_url() . 'blog/tag/' . $data['tag']->tag_slug, $this->blog_model->get_paginated_tag_posts_count($data['tag']->tag_slug), $this->blog_paginate_per_page);
    ...
    public function post($category_slug, $slug)
    {
        ...
        $data['og_type'] = "article";
        $data['og_url'] = base_url() . "blog/" . $data['post']->category_slug . "/" . $data['post']->slug;
    ...
    //get sub categories
    public function get_third_categories()
    {
        $id = $this->input->post('parent_id', true);
        $data["subcategories"] = $this->category_model->get_subcategories_by_parent_id($id);
    ...
    //set site language
    public function set_siste_language()
    {
    ...
    public function set_default_location()
    {
        ...
        if ($location_id == "all") {
            if (isset($_SESSION["modesy_default_location"])) {
                unset($_SESSION["modesy_default_location"]);
            }
        } else {
            $_SESSION["modesy_default_location"] = $location_id;
        }
    }
...
$country_name = $this->ip_info("Visitor","Country");
$country_id = $this->location_model->country_by_name($country_name);
return $country_id;
}
}
