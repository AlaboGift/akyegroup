<div class="row table-filter-container">
    <div class="col-sm-12">
        ...
        <div class="item-table-filter">
            <label>Type of Requested Service</label>
            <select id="categories" name="service_type" class="form-control" onchange="get_service(this.value);">
                <option value=""><?php echo trans("all"); ?></option>
                <?php
                $categories = $this->product_model->get_services();
                foreach ($categories as $item): ?>
                    <option value="<?=$item->profession?>"><?=html_escape($item->profession); ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
